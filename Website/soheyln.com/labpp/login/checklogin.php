...
if(isset($_POST['login']))
{
    ...
    if(mysql_num_rows($run))
    {
		...
		if ($rememberme == "yes") //if the Remember me is checked, it will create a cookie.
		{
			...
			//setcookie("user_email", $user_email, false, "/", ".soheyln.com"); //86400 = 1 day
			//setcookie("user_pass", md5($user_pass), false, "/", ".soheyln.com"); //86400 = 1 day
			//ob_start(); //set output buffering before setting cookies..
        ...
        //echo "<script>window.open('http://soheyln.com/labpp/index.php?email=" . $user_email . "','_blank')</script>";
        //echo "<script>window.open('http://soheyln.com/labpp/index.php','_blank')</script>";
		
		$row = mysql_fetch_assoc($run);
		if ($row['user_active']==1)
			echo '<script type="text/javascript">window.open("http://soheyln.com/labpp","_top");
		...
		else
		{
			echo '<center></br></br>
					...
					<h3 id="counterdown" style="color:red">8</h3>
				</center>' . 
				'<script>
					...
					var countdown = setInterval(function(){
						...
						if (count == 0) {
							...
							//window.parent.location.reload();
							window.open("http://soheyln.com/labpp","_top");
						}
						count--;
					}, 1000);
